...
public:
    template <class... Args>
    /**
         * @brief Constructs the implementation and initializes the internal number identifier.
         *
         * Forwards all constructor arguments to initialize the contained NumberIdentifier instance.
         */
        explicit Impl(Args&&... args)
        : _numberIdentifier(std::forward<Args>(args)...) {}

    /**
     * @brief Detects and identifies armor plates in the input image by finding paired light bars and validating them with the number recognizer.
     *
     * @param img Input BGR image used for light-bar detection and number recognition.
     * @param target_color Team color to match for valid light bars.
     * @param blacklist Bitmask specifying armor IDs to ignore during number recognition.
     * @return std::vector<ArmorPlate> Vector of armor plates that passed geometric filters and whose numbers were successfully identified; each ArmorPlate contains its light bars, computed center, and identified ID (or Unknown if not assigned).
     */
...
private:
    ...
    /**
     * @brief Computes a dimensionless measure of lateral misalignment between two light bars.
     *
     * @param LBl Left light bar.
     * @param LBr Right light bar.
     * @return double A non-negative scalar that quantifies how much the two light bars are misaligned along their combined axis; smaller values indicate better alignment.
     */
    ...
    template <typename T>
    /**
     * @brief Clamp a value in-place to the closed interval [min, max].
     *
     * @tparam T Numeric or comparable type of the value and bounds.
     * @param[in,out] value Reference to the value to be clamped; updated to lie within the range.
     * @param min Lower bound of the allowed range (inclusive).
     * @param max Upper bound of the allowed range (inclusive).
     */
    ...
    /**
     * @brief Attempts to convert a contour into a LightBar candidate matching target color.
     *
     * Validates contour geometry, fitted line orientation, filled coverage, width/length ratio,
     * and color polarity within the contour's bounding region. If the contour passes all checks,
     * returns a LightBar containing the top and bottom points and the bar angle.
     *
     * @param img Source BGR image used for color validation and ROI extraction.
     * @param contour Contour points describing the candidate light region.
     * @param target_color Desired robot color (BLUE or RED) used to check color dominance.
     * @return std::optional<LightBar> A LightBar when the contour represents a valid light bar for the
     * target color, `std::nullopt` otherwise.
     */
...
/**
     * @brief Constructs an ArmorIdentifier using the specified number-recognition model.
     *
     * Initializes internal implementation and loads the model located at the given filesystem path.
     *
     * @param model_path Filesystem path to the number-recognition model used by the internal NumberIdentifier.
     */
    ArmorIdentifier::ArmorIdentifier(const std::string& model_path)
    : pImpl_(new Impl{model_path}) {}

/**
 * @brief Identify armor plates visible in an image.
 *
 * Processes the provided image to detect and recognize armor plates matching the specified team color,
 * excluding any IDs indicated by the blacklist bitmask.
 *
 * @param img Source image in which to detect armor plates.
 * @param target_color Team color of the target armor plates (e.g., blue or red).
 * @param blacklist Bitmask of armor IDs to ignore; a set bit for an ID prevents that ID from being reported.
 * @return std::vector<ArmorPlate> Vector of identified and recognized armor plates found in the image.
 */
...
/**
 * @brief Destroys the ArmorIdentifier and releases associated resources.
 *
 * Performs default cleanup of the internal implementation and any held resources.
 */
