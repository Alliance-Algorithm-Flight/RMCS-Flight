...
/**
 * @brief Constructs a NumberIdentifier by loading an ONNX model.
 *
 * Loads the neural network model from the specified file path into the internal DNN network instance.
 *
 * @param model_path Filesystem path to the ONNX model file.
 * @throws std::runtime_error If the model cannot be read or the loaded network is empty.
 */
...
/**
 * @brief Identifies the armor type shown in a grayscale image region and updates the provided ArmorPlate.
 *
 * Processes the provided grayscale image of an armor candidate, runs a trained ONNX classifier, and if the predicted class passes whitelist checks and meets the confidence threshold, sets armor.id and armor.is_large_armor accordingly.
 *
 * @param imgGray Grayscale source image containing the armor candidate (full image; region-of-interest is derived from armor.points).
 * @param armor Input/output ArmorPlate: on success this object's `id` and `is_large_armor` are set to the identified type and size.
 * @param whitelist Bitmask of allowed/blocked armor types used to accept or reject specific predicted classes.
 *
 * @return `true` if an armor type was identified, passed whitelist checks, and the classifier confidence was at least 0.85; `false` otherwise.
 */
