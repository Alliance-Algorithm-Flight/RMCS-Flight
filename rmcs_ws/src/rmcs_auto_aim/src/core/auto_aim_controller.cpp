...
public:
    /**
     * @brief Constructs and initializes the AutoAimController node and its subsystems.
     *
     * Initializes the ROS2 node, registers required inputs and outputs, configures the camera
     * capturer and image viewer, reads and applies camera intrinsics and distortion coefficients,
     * and loads control parameters used by the aiming and firing logic.
     */
    AutoAimController()
        : rclcpp::Node(
              get_component_name(),
    ...
    /**
     * @brief Stops background processing, joins worker threads, and logs destruction.
     *
     * Sets the internal run flag to false to request worker threads to exit, joins any
     * still-running threads in the threads_ vector to ensure clean shutdown, and emits
     * an informational log message indicating the controller was destroyed.
     */
    ...
    /**
     * @brief Perform a single control update cycle for the auto-aim controller.
     *
     * Updates internal TF buffer, optionally starts the background vision worker on first invocation,
     * reads the latest tracked target frame, computes an aiming direction and firing decision, and
     * writes outputs (control_direction_ and fire_control_) and debug values (debug_target_omega_,
     * debug_target_theta_). If no valid target is available or the target frame is stale, the
     * controller clears aiming and disables firing.
     *
     * The first call that meets startup conditions (run_flag_ false and update_count_ == 0)
     * launches a worker thread that captures images, identifies armors, runs 3D fusion and tracking,
     * and populates the armor target buffer consumed by this method.
     *
     * @throws std::runtime_error If startup is attempted but target_color_ is not ready.
     */
